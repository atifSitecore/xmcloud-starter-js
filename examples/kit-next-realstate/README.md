# Real Estate - Demo Site (NextJS) - kit-next-realstate
...
2. Starting from the root of the repository navigate to site app folder
    ```cd examples\kit-next-realstate\```
...
6. Install dependencies:
   from ```kit-next-realstate``` run ```npm install```
...
6. Provide Editing host name  ```kit-next-realstate``` as per xmcloud.build.json
